...
internal class Server
{
    ...
    private static void ClientProcess(object? state)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                else if (mask)
                {
                    ...
                    var result = Encoding.UTF8.GetBytes(MatrixMultiply(text));

                    var header = GetHeader(true, false);

                    header = (header << 7) + result.Length;
    ...
    private static string MatrixMultiply(string text)
    {
        var j = default(int);

        var matrix_1_string = text.Split("\r\n")[0].Split(" ");
        var matrix_2_string = text.Split("\r\n")[1].Split(" ");

        var matrix_1 = new float[16];
        var matrix_2 = new float[16];

        for (var i = 0; i < 16; ++i)
        {
            matrix_1[i] = float.Parse(matrix_1_string[i]);
            matrix_2[i] = float.Parse(matrix_2_string[i]);
        }

        var resultMatrix = new float[16];
        ...
        for (var rowIndex = 0; rowIndex < 4; ++rowIndex)
        {
            for (var columnIndex = 0; columnIndex < 4; ++columnIndex)
            {
                ...
                for (var i = 0; i < 4; ++i)
                {
                    value += matrix_1[rowIndex * 4 + i] * matrix_2[i * 4 + columnIndex];
                }

                resultMatrix[rowIndex * 4 + columnIndex] = value;
            }
        }
        ...
        for (var i = 0; i < 4; ++i)
        {
            for (var k = 0; k < 4; ++k)
            {
                result += resultMatrix[i * 4 + k].ToString();
